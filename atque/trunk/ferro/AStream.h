...
public:
	...
	AOStream&
	write(const char *__ptr, uint32 __count) throw(AStream::failure);
	
	AOStream&
	write(const unsigned char * __ptr, uint32 __count) throw(AStream::failure)
	{ return write((char *) __ptr, __count); }
	
	AOStream&
	write(const signed char * __ptr, uint32 __count) throw(AStream::failure)
	{ return write((char *) __ptr, __count); }
	...
	template<class T>
	inline AOStream&
	write(const T* __list, uint32 __count) throw(AStream::failure)
	{
		T* ValuePtr = __list;
		for (unsigned int k=0; k<__count; k++)
			*this << *(ValuePtr++);
